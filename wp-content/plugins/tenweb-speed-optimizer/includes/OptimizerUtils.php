...
class OptimizerUtils
{
    ...
    public static function delete_all_cache_db($args = [])
    {
        ...
        $css = $wpdb->delete($tbl, array('meta_key' => 'two_cache_css')); // phpcs:ignore
        $js = $wpdb->delete($tbl, array('meta_key' => 'two_cache_js')); // phpcs:ignore
        $gzip = $wpdb->delete($tbl, array('meta_key' => 'two_cache_gzip')); // phpcs:ignore
    ...
    public static function clear_iframe_src($content){
        ...
        $content = OptimizerUtils::inject_in_html($content, '<script '.esc_attr(OptimizerScripts::TWO_DISABLE_PAGESPEED_DEFER_ATTRIBUTE).' '.esc_attr(OptimizerScripts::TWO_NO_DELAYED_JS_ATTRIBUTE).' src="' . plugins_url('external/js/two_elementor_video_to_iframe.js', __FILE__) . '"></script>', array('</body>', 'before'));
    ...
    public static function get_modes($name = null , $preview = false , $level = null){
        ...
        if($preview){
            foreach ($modes as $key=>$mode){
                $modes[$key]["preview_url"] =  add_query_arg(array('two_preview' => '1' , 'two_level'=>$mode["level"]), get_home_url() . "/");
            }
            return  $modes;
        }
        if(isset($name)){
    ...
    public static function triggerPostOptimizationTasks($disabled_incompatible_plugins = array())
    {
    ...
    public static function check_score()
    {
        ...
        if($access_token && $domain_id && $workspace_id) {
            wp_remote_post( TENWEB_SO_CRITICAL_URL."/v1/workspaces/".$workspace_id."/performance/".$domain_id."/googlepagespeed", array(
                'redirection' => 15,
                'blocking'    => false,
                'headers'     => array(
                    "accept" => "application/x.10webperformance.v1+json",
    ...
    public static function get_permalink_name_by_id( $id = 'front_page' ) {
        if ( 'front_page' === $id ) {
            return array( 'url' => home_url(), 'title' => 'Home' );
        }
        else {
            if ( 0 === strpos( $id, 'term_' ) ) {
                $term_id = (int) ltrim ( $id, 'term_' );
                $term = get_term( $term_id );
                return array( 'url' => get_term_link( $term_id ), 'title' => $term->name );
            }
            else if ( 0 === strpos( $id, 'user_' ) ) {
                $user_id = (int) ltrim ( $id, 'user_' );
                return array( 'url' => get_author_posts_url( $user_id ), 'title' => get_the_author_meta( 'nicename', $user_id ) );
            }
            else {
                return array( 'url' => get_permalink( $id ), 'title' => get_the_title( $id ) );
            }
        }
    }
    ...
    public static function get_post_id($page_url = null) {
        $pageUrlHash = md5($page_url); // this function is called in a loop to replace backgrounds, in order not to make a thousand requests to website to find page_id we will cache it in globals
        ...
        if( !empty( $page_url ) ) {
            ...
            if ( 0 === $id ) {
                $page_headers = wp_get_http_headers( add_query_arg( array( 'two_detect_post_id' => '1' ), $page_url ) );
    ...
    public static function two_update_subscription(){
        ...
        if(!empty($access_token) && !empty($domain_id) && !empty($workspace_id)) {
            ...
            } else if (is_wp_error( $response ) || !isset($response["body"], $response["response"]["code"]) || $response["response"]["code"] !== 429) {
    ...
    public static function download_critical(){
        if(isset($_GET["two_update_critical"], $_GET["page_id"]) && $_GET["two_update_critical"]==="1"){ // phpcs:ignore
            ...
            $domain_id = get_site_option('tenweb_domain_id');
            $access_token = get_site_option(TENWEB_PREFIX . '_access_token');
            $file_content_response = wp_remote_get( TENWEB_SO_CRITICAL_URL."/v1/critical/".$domain_id."/pages/".$page_id."/get", array( // phpcs:ignore
                'timeout'     => 5, // phpcs:ignore
                'redirection' => 5,
                'httpversion' => '1.0',
                'blocking'    => true,
    ...
    public static function update_connection_flow_progress($status, $step, $metaData = []){
        ...
        $access_token = get_site_option(TENWEB_PREFIX . '_access_token');
        if($access_token && $domain_id) {
            $response = wp_remote_post( TENWEB_SO_CRITICAL_URL."/v1/domains/".$domain_id."/update_connection_flow_progress", array(
    ...
    public static function injectCriticalBg($content , $critical, $cacheStructure){
        ...
        if(isset($critical_bg_css) && !empty($critical_bg_css)){
            $content = OptimizerUtils::inject_in_html($content, $critical_bg_css, array('</head>', 'before'));
            $cacheStructure->addToTagsToAdd($critical_bg_css, array('</head>', 'before'));
        }
        return $content;
    }
    ...
    public static function clear_cloudflare_cache($prefixes = array(), $clear_cache_from = ''){
        global $TwoSettings;
        ...
        if(isset($access_token, $domain_id, $workspace_id) && !empty($access_token) && !empty($domain_id) && !empty($workspace_id)){
            $response = wp_remote_post( TENWEB_SO_CRITICAL_URL."/v1/workspaces/".$workspace_id."/domains/".$domain_id."/clear_cloudflare_cache", array(
                'timeout'     => 1,
                'redirection' => 5,
                'httpversion' => '1.0',
                'blocking'    => false,
    ...
    public static function delete_so_page($page_id){
        ...
        $workspace_id = get_site_option(TENWEB_PREFIX . '_workspace_id');
        if($access_token && $domain_id && $workspace_id) {
            wp_remote_post( TENWEB_SO_CRITICAL_URL."/v1/workspaces/".$workspace_id."/domains/".$domain_id."/delete-so-page", array(
                'timeout'     => 5, // phpcs:ignore
                'redirection' => 5,
                'httpversion' => '1.0',
                'blocking'    => true,
    ...
    public static function init_flow_score_check($only_do_request = false){
        ...
        if($flow_score_check_init !== "1" && !is_array($two_flow_speed) && !\Tenweb_Authorization\Login::get_instance()->check_logged_in()){
            ...
            $speed_data["nooptimize_score"] = array();
            ...
            $speed_data["nooptimize_score"]["desktopData"] = array();
            $speed_data["nooptimize_score"]["mobileData"] = array();
    ...
    public static function send_data_to_performance($route, $body, $access_token){
        $res = wp_remote_post( $route, array(
    ...
    public static function warmup_cache()
    {
        if (!self::check_if_hosted_website()) {
            ...
            wp_remote_get($site_url, array('sslverify' => false,'blocking' => false, 'timeout' => 0.1)); // phpcs:ignore
        }
