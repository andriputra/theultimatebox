...
class OptimizerScripts extends OptimizerBase
{
    ...
    public function getcontent() {
        ...
        if (!empty($this->two_js_list) && $this->delay_all_js_execution ) {
            $two_events_after_load = $this->TwoSettings->get_settings("two_events_after_load", array());
            $dispatchEvents = '';
        ...
        // phpcs:ignore Squiz.PHP.CommentedOutCode.Found
        $this->content = OptimizerUtils::inject_in_html($this->content, $two_dispatchEvent_script, array( '</body>', 'before' ));
        $this->cacheStructure->addToTagsToAdd($two_dispatchEvent_script, array( '</body>', 'before' ));
