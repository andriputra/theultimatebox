...
class OptimizerAdmin
{
    ...
    public function checkDiviSettingsChange() {
        ...
        if ( get_option(TW_OPTIMIZE_PREFIX . '_clear_cache_after_divi')
            ||  ( !empty( $newCustomCSS ) && get_option( TW_OPTIMIZE_PREFIX . '_old_divi_custom_css' ) !== $newCustomCSS ) ) {
    ...
    public function init_admin()
    {
        ...
        if (!isset($_GET[ "two_nooptimize" ]) && !isset($_GET[ "two_action" ]) && current_user_can('manage_options')) {
            ...
            add_action('admin_init', array($this, 'checkDiviSettingsChange'));
            ...
            add_action('wp_ajax_two_elementor_regenerate_ccss', array($this, 'two_elementor_regenerate_ccss'));
    ...
    */
    ...
    public function admin_menu()
    {
        ...
        if (!defined('TWO_INCOMPATIBLE_ERROR')
            ...
            if (!\TenWebOptimizer\OptimizerUtils::is_paid_user()) {
                if (get_site_option(TENWEB_PREFIX . '_client_referral_hash')) {
                    $first_visit_referral = 'two_referral_first_visit';
                    if (get_option('two_referral_visited')) {
                        $first_visit_referral = 'two_referral_visited';
                    }
                    add_submenu_page(
                        'two_settings_page',
                        'Referral program',
                        '<span class="' . sanitize_html_class($first_visit_referral) . '">Referral program</span>',
                        'manage_options',
                        'two_referral_program',
                        array(
                            '\TenWebOptimizer\OptimizerAdmin',
                ...
                elseif ($task === "update_defaults"){
                    if(isset($_POST["data"]["default_settings"])){
                        ...
                        if(empty($_POST["data"]["default_settings"]["default_sizes"])){
                            $default_settings["default_sizes"] = array();
                        }
...
//        }
        ...
        if ($clear_two_cloudflare_cache) {
            OptimizerUtils::clear_cloudflare_cache(array(), $clear_cache_from);
        }
    ...
    private static function set_additional_settings()
    {
        ...
        if (!get_option('two_set_dispatch_events')) {
            $TwoSettings->update_setting('two_events_after_load',array("DOMContentLoaded","Load","Click"));
    ...
    public static function setFlowIdNotificationId()
    {
        ...
        if ($domain_id) {
            ...
            $response = wp_remote_post(TENWEB_API_URL . '/domains/' . $domain_id . '/products/run-booster-flow', array(
                'method' => 'POST',
            ...
            if (!is_wp_error($response) && isset($response["body"], $response["response"]["code"]) && $response["response"]["code"] === 200) {
                ...
                update_option('two_connection_after_io',array($response), false);
            }
        }
        self::startTheFlow();
    }
