...
class OptimizerApi {
    ...
    public function two_rest(){
        ...
        register_rest_route('tenweb_so/v1', 'two_settings',
            array(
                'methods'  => 'GET',
                'callback' => array($this, "get_two_settings"),
                'permission_callback' => array($this, 'check_authorization'),
                'args'     => [
                    'filter' => [
                        'type'     => 'string',
        ...
        register_rest_route('tenweb_so/v1', 'two_settings',
            array(
                'methods'  => 'POST',
                'callback' => array($this, "set_two_settings"),
                'permission_callback' => array($this, 'check_authorization'),
                'args'     => [
                    'settings' => [
                        'type'     => 'json',
        ...
        register_rest_route('tenweb_so/v1', 'regenerate_critical',
            array(
                'methods'  => 'POST',
                'callback' => array($this, "regenerate_critical"),
                'permission_callback' => array($this, 'check_authorization'),
                'args'     => [
                    'page_id' => [
                        'type'     => 'txt',
        ...
        register_rest_route('tenweb_so/v1', 'regenerate_webp',
            array(
                'methods'  => 'POST',
                'callback' => array($this, "regenerate_home_webp"),
                'permission_callback' => array($this, 'check_authorization'),
            )
        );

        register_rest_route('tenweb_so/v1', 'delete_webp_images',
            array(
                'methods'  => 'POST',
                'callback' => array($this, "delete_webp_images"),
                'permission_callback' => array($this, 'check_authorization'),
            )
        );
    }
                ...
                foreach ($settings as $key=>$val) {
                    ...
                    if(isset($settings_names[$key])){
                        if($this->validate_accepted_value($settings_names[$key], $val)){
                            if($key === "two_critical_status" && $val === "on"){
                                $val = "true";
                            }
                            if($key === "two_delay_all_js_execution" && $val === "off"){
                                $val = "";
                            }
